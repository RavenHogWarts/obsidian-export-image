...
const zh = {
  ...
  setting: {
    ...
    split: {
      ...
      mode: {
        label: "拆分模式",
        description:
          "选择是否拆分图片，以及如何拆分。固定高度表示每个拆分图片的高度固定，可能在拆分处截断文字，按分隔线拆分表示按文档中的 hr 元素拆分，按段落自动拆分表示拆分时确保一个段落不会拆分到两张图片中，且尽可能接近拆分高度。",
        none: "不拆分",
        fixed: "固定高度",
        hr: "按分隔线拆分",
        auto: "按段落自动拆分",
      ...
      overlap: {
        label: "拆分图片重叠",
        description:
          "设置相邻两个拆分图片的重叠（像素），防止内容被截断。默认为 40px。",
      },
    },
    filename: {
      label: "是否包含文件名作为标题",
    ...
    quickExportSelection: {
      label: "快速导出选中内容",
      description:
        "导出选中内容时是否要跳过配置过程，直接把导出的图片复制到剪切板。",
    },
    userInfo: {
      title: "作者信息",
      show: "显示作者信息",
    ...
    customCSS: {
      ...
      css: {
        label: "CSS 文件",
        description:
          "设置加载的自定义 CSS 文件。CSS 选择器应以 .export-image-root 为前缀以确保正确作用域。",
        default: "默认样式",
      },
    },
  },
  ...
  imageUrl: "输入 URL",
  splitInfo:
    "图片总高度为 {rootHeight}px，分割高度为 {splitHeight}px，因此将生成 {pages} 张图片",
