...
const ko = {
  ...
  guide:
    "이동하려면 드래그하고, 확대/축소하려면 스크롤하거나 핀치하세요, 더블 클릭으로 초기화합니다.",
  ...
  setting: {
    ...
    split: {
      ...
      mode: {
        label: "분할 모드",
        description:
          "이미지를 분할하는 방법을 선택합니다. 고정 높이는 각 분할된 페이지가 고정된 높이를 가지며, 텍스트가 분할 지점에서 잘릴 수 있습니다. 수평 라인 분할은 문서 내의 수평 라인에 따라 이미지를 분할합니다. 문단 분할은 문단에 따라 이미지를 분할하여 문단이 두 개의 이미지로 분할되지 않도록 하고 분할 높이에 가장 가까운 높이를 유지합니다.",
        none: "분할 없음",
        fixed: "고정 높이",
        hr: "수평 라인",
        auto: "문단",
      ...
      height: {
        label: "페이지 높이",
        description:
          "각 분할된 페이지의 높이를 픽셀 단위로 설정합니다. 기본값은 1000px입니다.",
      },
      overlap: {
        label: "중첩 여백",
        description:
          "페이지 간의 중첩 여백을 설정하여 내용이 갑자기 끊기는 것을 방지합니다. 기본값은 40px입니다.",
      },
    },
    filename: {
    ...
    quickExportSelection: {
      ...
      description:
        "선택 내용을 내보낼 때 설정 프로세스를 건너뛰고 내보낸 이미지를 직접 클립보드에 복사합니다.",
    },
    userInfo: {
      title: "작성자 정보",
      show: "작성자 정보 표시",
  ...
  imageUrl: "이미지 URL",
  splitInfo:
    "이미지 총 높이는 {rootHeight}px이고, 분할 높이는 {splitHeight}px이므로, {pages}개의 이미지가 생성됩니다.",
  splitInfoHr:
    "이미지 총 높이는 {rootHeight}px이고, 분할 높이는 {splitHeight}px이므로, {pages}개의 이미지가 생성됩니다.",
